...
bool ros2web_interfaces__msg__ws_msg_data__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("ros2web_interfaces.msg._ws_msg_data.WSMsgData", full_classname_dest, 45) == 0);
  }
  ros2web_interfaces__msg__WSMsgData * ros_message = _ros_message;
  {  // str
  ...
  {  // bytes
    PyObject * field = PyObject_GetAttrString(_pymsg, "bytes");
    if (!field) {
      return false;
    }
    if (PyObject_CheckBuffer(field)) {
      ...
      uint8_t * dest = ros_message->bytes.data;
      ...
      PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'bytes'");
      if (!seq_field) {
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of WSMsgData */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("ros2web_interfaces.msg._ws_msg_data");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "WSMsgData");
  ...
  {  // bytes
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "bytes");
    if (!field) {
      return NULL;
    }
    ...
    if (ros_message->bytes.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      uint8_t * src = &(ros_message->bytes.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->bytes.size * sizeof(uint8_t));
      assert(data != NULL);
